...
Real-ESRGAN ベースの画像・動画処理ツール（アップスケール・形式変換・軽量化・背景削除・CTAアニメーション）
...
- **CTAアニメーション**: 画像に動きを付けて GIF または LP 埋め込み用ソースコードを生成（21種類のエフェクト、スピード・強さ・ループ間隔を数値で細かく指定可能、リアルタイムプレビュー）
- **パイプライン**: 複数処理を連続実行（例: 背景削除 → トリミング → アップスケール）
